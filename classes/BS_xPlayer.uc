...
simulated function MakeSureSaveConfig()
{
  ...
  bStats = default.bStats;
  bEnableUTCompAutoDemorec = default.bEnableUTCompAutoDemorec;
  DemoRecordingMask = default.DemoRecordingMask;
  bEnableAutoScreenshot = default.bEnableAutoScreenshot;
  ScreenShotMask = default.ScreenShotMask;
  FriendlySound = default.FriendlySound;
  EnemySound = default.EnemySound;
  bEnableHitSounds = default.bEnableHitSounds;
  HitSoundVolume = default.HitSoundVolume;
  bCPMAStyleHitsounds = default.bCPMAStyleHitsounds;
  CPMAPitchModifier = default.CPMAPitchModifier;
  SavedSpectateSpeed = default.SavedSpectateSpeed;
  bUseDefaultScoreBoard = default.bUseDefaultScoreBoard;
  bShowSelfInTeamOverlay = default.bShowSelfInTeamOverlay;
  bEnableEnhancedNetCode = default.bEnableEnhancedNetCode;
  bEnableColoredNamesOnEnemies = default.bEnableColoredNamesOnEnemies;

  ballowcoloredmessages = default.ballowcoloredmessages;
  bEnableColoredNamesInTalk = default.bEnableColoredNamesInTalk;

  LoadedEnemySound = Sound(DynamicLoadObject(default.EnemySound, class'Sound', True));
  LoadedFriendlySound = Sound(DynamicLoadObject(default.FriendlySound, class'Sound', True));




  CurrentSelectedColoredName = default.CurrentSelectedColoredName;
  for(i=0; i<ArrayCount(ColorName); i++)
  {
      ColorName[i] = default.ColorName[i];
  }
  ColoredName =  default.ColoredName;
}
...
simulated function InitializeStuff()
{
    ...
    SetShowSelf(default.bShowSelfInTeamOverlay);
    ...
    if(default.bFirstRun)
    {
        ...
        {
           class'UTComp_xPawn'.default.bRedTeammateModelsForced=False;
           class'UTComp_xPawn'.default.bBlueEnemyModelsForced=False;
           class'UTComp_xPawn'.Static.StaticSaveConfig();
        }
        else
        {
            class'utcomp_xpawn'.default.BlueEnemyModelName=class'xGame.xPawn'.default.PlacedCharacterName;
            class'utcomp_xpawn'.default.RedTeammateModelName=class'xGame.xPawn'.default.PlacedCharacterName;
            class'UTComp_xPawn'.static.staticsaveconfig();
        }
...
simulated function InitializeScoreboard()
{
   ...
   {
       if(default.bUseDefaultScoreboard)
       {
           if(gamereplicationinfo.bTeamGame)
               NewScoreboardClass=class<Scoreboard>(DynamicLoadObject("UTCompvSrc.UTComp_ScoreBoardTDM", class'Class'));
           else
               NewScoreboardClass=class<Scoreboard>(DynamicLoadObject("UTCompvSrc.UTComp_ScoreBoardDM", class'Class'));
           ClientChangedScoreboard=True;
        }
   }
   else if(ClientChangedScoreBoard && !default.bUseDefaultScoreboard)
       NewScoreboardClass=class<Scoreboard>(DynamicLoadObject("UTCompvSrc.UTComp_ScoreBoard", class'Class'));
...
{
    if(default.currentselectedcoloredname!=255 && default.currentselectedcoloredname<default.ColoredName.Length)
    {
        SetColoredNameOldStyleCustom(,default.currentselectedcoloredname);
    }
    else
...
{
    local string S;
    S=StripIllegalWindowsCharacters(default.DemoRecordingmask);

    if (default.bEnableUTCompAutoDemorec && (level.NetMode!=NM_DedicatedServer))
    {
...
state GameEnded
{
    ...
    function Timer()
    {
        if (default.bEnableAutoScreenShot && screenshotstaken==0)
        {
            ConsoleCommand("shot "$StripIllegalWindowsCharacters(default.ScreenShotMask));
            screenshotstaken++;
        }
        Super.Timer();
    }
...
simulated function ReceiveHit(class<DamageType> DamageType, int Damage, pawn Injured)
{
    ...
    else if(Injured.GetTeamNum()==255 || (Injured.GetTeamNum() != GetTeamNum()))
    {
        ...
        if(default.bCPMAStyleHitsounds && (DamageType == class'DamTypeFlakChunk' || DamageType == class'DamTypeFlakShell') && (RepInfo==None || RepInfo.EnableHitSoundsMode==2 || LineOfSightTo(Injured)))
        {
...
simulated function PlayEnemyHitSound(int Damage)
{
    ...
    if(!default.bEnableHitSounds || LastHitSoundTime>Level.TimeSeconds)
        return;
    LastHitSoundTime=Level.TimeSeconds+HITSOUNDTWEENTIME;
    HitSoundPitch=1.0;
    if(default.bCPMAStyleHitSounds)
        HitSoundPitch=default.CPMAPitchModifier*30.0/Damage;
    if(LoadedEnemySound == none)
        LoadedEnemySound = Sound(DynamicLoadObject(default.EnemySound, class'Sound', True));

    if(ViewTarget!=None)
        ViewTarget.PlaySound(LoadedEnemySound,,default.HitSoundVolume,,,HitSoundPitch);
}
...
simulated function PlayTeammateHitSound(int Damage)
{
    ...
    if(ViewTarget!=None)
        ViewTarget.PlaySound(LoadedFriendlySound,,default.HitSoundVolume,,,HitSoundPitch);
}
...
exec function voteyes()
{
    if(UTCompPRI!=None && UTCompPRI.CurrentVoteID!=255)
    {
        ...
        if(PlayerReplicationInfo!=None && !PlayerReplicationInfo.bOnlySpectator && Level.TimeSeconds > LastBroadcastVoteTime)
        {
            BroadcastVote(True);
...
simulated function CallVote(byte b, optional byte p, optional string Options, optional int p2, optional string Options2)
{
    ...
    if(PlayerReplicationInfo!=None && UTCompPRI!=None && PlayerReplicationInfo.bAdmin)
        UTCompPRI.PassVote(b,p,Options,p2,Options2);
    if(UTCompPRI!=None)
        UTCompPRI.CallVote(b,p, Options, p2, Options2);
}
...
function bool IsValidVote(byte b, byte p, out string S, string S2)
{
    ...
    if(uWarmup==None)
       foreach DynamicActors(class'UTComp_Warmup', uWarmup)
           break;
    if(Level.Game.IsA('UTComp_Duel') && (b==6 || b==7 || b==5) && (uWarmup==None || !uWarmup.bInWarmup))
    {
        ClientMessage("Sorry, in duel mode you may only call votes during warmup.");
        return false;
    }
    if(b>9 || b<0)
    {
...
exec function SetName(coerce string S)
{
   ...
   default.CurrentSelectedColoredName=255;
...
simulated function SetColoredNameOldStyle(optional string S2, optional bool bShouldSave)
{
    ...
    {
        numdoatonce=1;
        for(m=k;m<Len(S2)&& default.ColorName[k-1] == default.ColorName[m] ;m++)
        {
             numdoatonce++;
             k++;
        }
        S=S$class'UTComp_Util'.Static.MakeColorCode(default.ColorName[k-1])$Right(Left(S2, k), numdoatonce);
    }
    if(UTCompPRI!=None)
...
simulated function string FindColoredName(int CustomColors)
{
    ...
    if(S2=="")
    {
       S2=default.ColoredName[CustomColors].SavedName;
    }
    SetNameNoReset(S2);
 ...
 //   Log(default.ColoredName[CustomColors].SavedColor[0].R@default.ColoredName[CustomColors].SavedColor[0].G@default.ColoredName[CustomColors].SavedColor[0].B);
    for(k=0; k<20; k++)
        default.ColorName[k]=default.ColoredName[CustomColors].SavedColor[k];
    for(k=1; k<=Len(S2); k++)
    {
        numdoatonce=1;
        for(m=k;m<Len(S2)&& default.ColoredName[CustomColors].SavedColor[k-1] == default.ColoredName[CustomColors].SavedColor[m] ;m++)
        {
             numdoatonce++;
             k++;
        }
        S=S$class'UTComp_Util'.Static.MakeColorCode(default.ColoredName[CustomColors].SavedColor[k-1])$Right(Left(S2, k), numdoatonce);
    }
    return S;
}
...
simulated function SaveNewColoredName()
{
    ...
    n=default.ColoredName.Length+1;
    default.ColoredName.Length=n;

 //   Log(default.ColoredName.Length);

    default.ColoredName[n-1].SavedName=PlayerReplicationInfo.PlayerName;

    for(l=0; l<20; l++)
         default.ColoredName[n-1].SavedColor[l]=default.ColorName[l];

    ColoredName.Length=default.ColoredName.Length;
    for(i=0; i<default.ColoredName.Length; i++)
        ColoredName[i]=Default.ColoredName[i];
    for(i=0; i<ArrayCount(ColorName); i++)
        ColorName[i]=Default.ColorName[i];
}
...
exec function ShowColoredNames()
{
    ...
    for(i=0; i<default.ColoredName.Length; i++)
    {
        Log(default.ColoredName[i].SavedName);
        for(j=0; j<20; j++)
            S=S$default.ColoredName[i].SavedColor[j].R@default.ColoredName[i].SavedColor[j].G@default.ColoredName[i].SavedColor[j].B;
        Log(S);
    }
}
...
exec function ListColoredNames()
{
   local int i;
   for(i=0; i<default.ColoredName.Length; i++)
       echo(default.coloredname[i].SavedName);
}

simulated function SetShowSelf(Bool b)
{
    bShowSelfInTeamOverlay=b;
    default.bShowSelfInTeamOverlay=b;
    staticsaveconfig();
    if(UTCompPRI!=None)
...
event TeamMessage( PlayerReplicationInfo PRI, coerce string S, name Type  )
{
    ...
    if(default.bAllowColoredMessages)
    {
       for(k=7; k>=0; k--)
       {
    ...
    if ( myHUD != None )
	{   if (default.bEnableColoredNamesInTalk)
...
simulated function MatchHudColor()
{
    ...
    if(!class'UTComp_xPAwn'.default.bEnemyBasedSkins)
    {
        if(class'UTComp_xPAwn'.default.ClientSkinModeRedTeammate == 3)
        {
            DMHud.HudColorRed=class'UTComp_xPawn'.default.RedTeammateUTCompSkinColor;
        }
        else if(class'UTComp_xPAwn'.default.ClientSkinModeRedTeammate == 2
            || class'UTComp_xPAwn'.default.ClientSkinModeRedTeammate == 1)
        {
            DMHud.HudColorRed=class'UTComp_xPawn'.default.BrightSkinColors[class'UTComp_xPawn'.default.PreferredSkinColorRedTeammate];
        }

        if(class'UTComp_xPAwn'.default.ClientSkinModeBlueEnemy == 3)
        {
            DMHud.HudColorBlue=class'UTComp_xPawn'.default.BlueEnemyUTCompSkinColor;
        }
        else if(class'UTComp_xPAwn'.default.ClientSkinModeBlueEnemy == 2
            || class'UTComp_xPAwn'.default.ClientSkinModeBlueEnemy == 1)
        {
            DMHud.HudColorBlue=class'UTComp_xPawn'.default.BrightSkinColors[class'UTComp_xPawn'.default.PreferredSkinColorBlueEnemy];
        }
    }
    else
    {
        if(class'UTComp_xPAwn'.default.ClientSkinModeRedTeammate == 3)
        {
            DMHud.HudColorBlue=class'UTComp_xPawn'.default.RedTeammateUTCompSkinColor;
            DMHud.HudColorRed=class'UTComp_xPawn'.default.RedTeammateUTCompSkinColor;
        }
        else if(class'UTComp_xPAwn'.default.ClientSkinModeRedTeammate == 2
            || class'UTComp_xPAwn'.default.ClientSkinModeRedTeammate == 1)
        {
            DMHud.HudColorBlue=class'UTComp_xPawn'.default.BrightSkinColors[class'UTComp_xPawn'.default.PreferredSkinColorRedTeammate];
            DMHud.HudColorRed=class'UTComp_xPawn'.default.BrightSkinColors[class'UTComp_xPawn'.default.PreferredSkinColorRedTeammate];
        }
    }
}
...
{
    return default.bEnableEnhancedNetCode;
}
...
defaultproperties
{
     ...
     FriendlySound="UTCompvSrc.Sounds.HitSoundFriendly"
     EnemySound="UTCompvSrc.Sounds.HitSound"
     ...
     bFirstRun=True
     UTCompMenuClass="UTCompvSrc.UTComp_Menu_OpenedMenu"
     UTCompVotingMenuClass="UTCompvSrc.UTComp_Menu_VoteInProgress"
     ...
     bEnableColoredNamesOnEnemies=false
}
