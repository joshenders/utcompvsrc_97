...
function int NetDamage( int OriginalDamage, int Damage, pawn injured, pawn instigatedBy, vector HitLocation, out vector Momentum, class<DamageType> DamageType )
{
    ...
    if(Damage>0 && InstigatedBy!=None && Injured!=None && InstigatedBy.Controller!=None && BS_xPlayer(InstigatedBy.Controller)!=None)
    {
        ...
        {
            if(BS_xPlayer(InstigatedBy.Controller).bStats && UTCompMutator.bEnableWeaponStats)
        ...
        else if(BS_xPlayer(InstigatedBy.Controller).bStats && UTCompMutator.bEnableWeaponStats)
        {
